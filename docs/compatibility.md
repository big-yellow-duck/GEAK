...
myst:
    html_meta:
        ...
        "keywords": "GEAK, compatibility, ROCm, AMD Instinct, RDNA4, gfx1201, Codex CLI, Claude Code, FlyDSL, vLLM"
---

# GEAK compatibility matrix

Verified hardware, software, runtime, and backend combinations for GEAK 4.0.0, a deterministic
JS-workflow GPU optimizer supporting Codex CLI and Claude Code. Only tested configurations are listed.
...
- `gfx1200` uses the same `gfx120x` wave32/WMMA routing but remains compile-supported rather than
...
|---|---|---|
| Codex CLI | Current CLI plus Node.js 18+ | Default local backend; authenticates with `codex login`. |
| Codex model | `gpt-5.6-sol` / `high` | Configurable with `GEAK_CODEX_MODEL` and `GEAK_CODEX_REASONING_EFFORT`. |
| Claude Code | ≥ 2.1.177 | Optional backend selected with `GEAK_AGENT_BACKEND=claude`. |
...
The profiler is auto-detected using an architecture-aware degrade ladder. RDNA4 prefers
`rocprofv3`/`rocprof`; CDNA retains `rocprof-compute` first.
...
| FlyDSL | Preferred dense/quantized GEMM author target. CDNA may use AITER integration; RDNA4 uses standalone upstream FlyDSL's native `gfx120x` wave32/WMMA path and must not import `aiter.ops.flydsl`. |
