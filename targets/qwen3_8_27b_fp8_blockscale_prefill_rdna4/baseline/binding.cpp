...
// Four native wave32 waves cooperate on one 128-column output tile.  The two
// waves in each pair own disjoint, balanced K-block ranges and are reduced in
// LDS. Repeating the single live A row via ldm=0 avoids the generic M=64
// padded-row work while preserving the native OCP-FP8 WMMA path.
__global__ __launch_bounds__(128)
void decode_splitk_wmma(const F8* __restrict__ a,
                        const F8* __restrict__ b,
                        const float* __restrict__ as,
                        const float* __restrict__ bs,
                        hip_bfloat16* __restrict__ out,
                        int m, int n, int k, int stride_b) {
    ...
    // [wave][16-column fragment][16x16 row-major matrix]
    ...
    for (int row = 0; row < 2; ++row) {
        ...
        for (int kb = kb_begin; kb < kb_end; ++kb) {
            ...
            for (int kk = 0; kk < 128; kk += 16) {
                FragA af;
                // ldm=0 intentionally broadcasts the one live decode row to
                // all 16 WMMA rows. Only row zero is consumed in the epilogue.
...
// The four smaller M=2 shapes benefit from a finer 64-column grid. Two waves
// retain the K split, but pack both live rows in one A fragment so the B
// traversal is shared. Only 1 KiB of LDS is needed for the final reduction.
__global__ __launch_bounds__(64)
void decode_splitk_wmma_m2(const F8* __restrict__ a,
                           const F8* __restrict__ b,
                           const float* __restrict__ as,
                           const float* __restrict__ bs,
                           hip_bfloat16* __restrict__ out,
                           int n, int k, int stride_b) {
    const int tid = threadIdx.x;
    const int wave = tid >> 5;
...
// Wide M=2 decode already has enough grid parallelism at one workgroup per
// 128 columns.  Give each of the four waves an independent 32-column slab and
// traverse all K blocks in every wave.  Both live A rows share one fragment,
// so each B fragment is consumed once; the two rows receive their independent
// activation scales after every exact 128-K FP32 partial.  Disjoint ownership
// also lets each wave write its two rows directly without LDS or barriers.
__global__ __launch_bounds__(128)
void decode_nosplit_wmma_m2_wide(const F8* __restrict__ a,
                                 const F8* __restrict__ b,
                                 const float* __restrict__ as,
                                 const float* __restrict__ bs,
                                 hip_bfloat16* __restrict__ out,
    ...
    for (int kb = 0; kb < scale_blocks; ++kb) {
        ...
        for (int kk = 0; kk < 128; kk += 16) {
            FragA af;
            rocwmma::fill_fragment(af, 0.0f);
            // gfx12 row-major A fragments are SOA: lane&15 selects the row
            // and lane>>4 selects one eight-element K half. Only rows 0 and 1
            // are populated, avoiding reads from nonexistent padded rows.
            const int arow = lane & 15;
            if (arow < 2) {
                const int ak = kbase + kk + (lane >> 4) * 8;
                af.mStorage = *reinterpret_cast<const FragA::Traits::StorageT*>(
...
torch::Tensor decode(torch::Tensor a, torch::Tensor weight,
                     torch::Tensor a_scale, torch::Tensor weight_scale) {
    ...
    if (m == 2 && n == 17408) {
        hipLaunchKernelGGL(decode_nosplit_wmma_m2_wide,
            dim3(n / 128), dim3(128), 0, stream,
            ...
            n, k, static_cast<int>(weight.stride(0)));
    } else if (m == 2) {
        hipLaunchKernelGGL(decode_splitk_wmma_m2,
            dim3(n / 64), dim3(64), 0, stream,
            reinterpret_cast<const F8*>(a.data_ptr()),
            reinterpret_cast<const F8*>(weight.data_ptr()),
            a_scale.data_ptr<float>(), weight_scale.data_ptr<float>(),
            reinterpret_cast<hip_bfloat16*>(out.data_ptr()),
        ...
        hipLaunchKernelGGL(decode_splitk_wmma,
            dim3(n / 128), dim3(128), 0, stream,
            reinterpret_cast<const F8*>(a.data_ptr()),
            reinterpret_cast<const F8*>(weight.data_ptr()),
            a_scale.data_ptr<float>(), weight_scale.data_ptr<float>(),
            reinterpret_cast<hip_bfloat16*>(out.data_ptr()),
            m, n, k, static_cast<int>(weight.stride(0)));
    }
    return out;
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("decode", &decode, "gfx1201 wave-paired split-K FP8 decode");
}
