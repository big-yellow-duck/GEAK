...
torch::Tensor decode(torch::Tensor a, torch::Tensor weight,
  ...
  if (m == 2 && n == 17408) {
    hipLaunchKernelGGL(decode_nosplit_wmma_m2_wide, dim3(n / 128), dim3(128), 0,
                       stream, reinterpret_cast<const F8 *>(a.data_ptr()),
    ...
    hipLaunchKernelGGL(decode_splitk_wmma, dim3(n / 128), dim3(128), 0, stream,
                       ...
                       a_scale.data_ptr<float>(),
                       weight_scale.data_ptr<float>(),
                       reinterpret_cast<hip_bfloat16 *>(out.data_ptr()), m, n,
                       k, static_cast<int>(weight.stride(0)));
  }
  return out;
}
